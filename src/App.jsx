...
// ═══════════════════════════════════════
// ═══════════════════════════════════════
// COLORS — Fundación Red Conecta
// ═══════════════════════════════════════
const C = {
  bg: "#0a1628", card: "#0f1f38", accent: "#00CED1", accentLight: "#33d9dc", accentDark: "#00b0b3",
  green: "#10b981", greenDark: "#059669", red: "#ef4444", blue: "#003366", coral: "#FF7F50",
  gold: "#FFD700", indigo: "#2E4053",
  text: "#f1f5f9", muted: "#94a3b8", dim: "#708090", border: "#1a2d4a", input: "#0b1526",
};
...
export default function App() {
  ...
  // Styles
  const container = { minHeight: "100vh", background: `linear-gradient(135deg,${C.bg} 0%,#0c1a30 50%,${C.bg} 100%)`, color: C.text, fontFamily: "'Montserrat','Segoe UI',system-ui,-apple-system,sans-serif" };
  ...
  const btnP = { width: "100%", padding: "14px 24px", borderRadius: 10, border: "none", background: `linear-gradient(135deg,${C.accent},${C.accentDark})`, color: "#003366", fontSize: 15, fontWeight: 700, cursor: "pointer", letterSpacing: .5, transition: "all .2s" };
  ...
  const handleLogin = async () => {
    ...
    if (!doc.trim() || !name.trim() || !institucion.trim()) { setError("Ingresa tu documento, nombre e institución."); return; }
    setLoading(true);
    let s = await fbGet(`students/${doc}`);
    if (!s) {
      s = { doc, name, institucion, salon, createdAt: new Date().toISOString(), pre: {}, post: {} };
    } else {
      s.name = name; s.institucion = institucion; s.salon = salon;
    }
  ...
  const exportCSV = () => {
    const rows = [["Documento", "Nombre", "Institucion", "Salon", "Momento", "Prueba", "Puntaje_Total", "Puntaje_Max", "Nivel", "Detalles", "Fecha"]];
      ...
      ["pre", "post"].forEach((m) => {
        ["rosenberg", "mspss", "aetdc"].forEach((t) => {
          if (s[m]?.[t]) {
            ...
            rows.push([s.doc, s.name, s.institucion || "", s.salon || "", m, t.toUpperCase(), sc.total, sc.max, sc.level, det, s[m][t].completedAt]);
          }
        });
      });
    });
    ...
    a.download = `resultados_red_conecta_${new Date().toISOString().slice(0, 10)}.csv`; a.click();
  };
  ...
  if (screen === "login") return (
    <div style={container}>
      <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;600;700;800&family=Lora:wght@400;600&display=swap" rel="stylesheet" />
      <div style={page}>
        <div style={{ textAlign: "center", paddingTop: 32, marginBottom: 32 }}>
          <div style={{ display: "inline-block", background: "#ffffff", borderRadius: 16, padding: "16px 28px", marginBottom: 16, boxShadow: "0 4px 24px rgba(0,206,209,0.15)" }}>
            <img src="/logo.png" alt="Fundación Red Conecta" style={{ maxWidth: 200, display: "block" }} />
          </div>
          <p style={{ color: C.accent, fontSize: 14, fontWeight: 600, marginBottom: 4 }}>Pruebas Diagnósticas</p>
          <p style={{ color: C.dim, fontSize: 12, fontFamily: "'Lora',serif" }}>Construyendo el puente entre el talento y el futuro profesional</p>
        </div>
        <div style={{ background: C.card, borderRadius: 16, padding: 28, border: `1px solid ${C.border}` }}>
          <label style={{ color: C.muted, fontSize: 12, fontWeight: 600, letterSpacing: 1, display: "block", marginBottom: 6 }}>INSTITUCIÓN EDUCATIVA</label>
          <input type="text" value={institucion} onChange={(e) => setInstitucion(e.target.value)} placeholder="Ej: I.E. Pablo Neruda"
          ...
          <label style={{ color: C.muted, fontSize: 12, fontWeight: 600, letterSpacing: 1, display: "block", marginBottom: 6 }}>CURSO / SALÓN</label>
          <input type="text" value={salon} onChange={(e) => setSalon(e.target.value)} placeholder="Ej: 11-01, 11-02"
            onKeyDown={(e) => e.key === "Enter" && handleLogin()}
            style={{ width: "100%", padding: "12px 16px", borderRadius: 8, border: `1px solid ${C.border}`, background: C.input, color: C.text, fontSize: 16, marginBottom: 24, outline: "none", boxSizing: "border-box" }} />
        ...
        <p style={{ textAlign: "center", color: C.dim, fontSize: 11, marginTop: 24, fontFamily: "'Lora',serif" }}>Fundación Red Conecta — El respaldo de tu talento</p>
      </div>
    </div>
  );
      ...
      <div style={container}>
        <div style={page}>
          <div style={header}>
            ...
            <p style={{ color: C.muted, fontSize: 13 }}>{student.institucion}{student.salon ? ` — ${student.salon}` : ""}</p>
          </div>
  ...
  if (screen === "admin") {
    ...
    const instituciones = [...new Set(students.map((s) => s.institucion || "Sin institución"))];
    const avg = (arr, mom, tst, field) => {
      const vals = arr.filter((s) => s[mom]?.[tst]?.scores).map((s) => +s[mom][tst].scores[field]);
    ...
    return (
      <div style={container}>
        ...
        <div style={{ ...page, maxWidth: 800 }}>
          <div style={header}>
            <div style={{ display: "inline-block", background: "#ffffff", borderRadius: 12, padding: "12px 20px", marginBottom: 12, boxShadow: "0 4px 24px rgba(0,206,209,0.15)" }}>
              <img src="/logo.png" alt="Fundación Red Conecta" style={{ maxWidth: 160, display: "block" }} />
            </div>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: C.accent }}>🔐 Panel Administrador</h2>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
            ...
            <ScoreCard label="Instituciones" value={instituciones.length} color={C.coral} />
          </div>

          {instituciones.map((inst) => {
            const instStudents = students.filter((s) => (s.institucion || "Sin institución") === inst);
            return (
              <div key={inst} style={{ background: C.card, borderRadius: 12, padding: 20, border: `1px solid ${C.border}`, marginBottom: 20 }}>
                <h3 style={{ fontSize: 14, fontWeight: 700, color: C.accent, marginBottom: 4 }}>🏫 {inst}</h3>
                <p style={{ color: C.dim, fontSize: 12, marginBottom: 12 }}>{instStudents.length} estudiantes</p>

                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, marginBottom: 12 }}>
                  <thead><tr style={{ color: C.muted }}>
                    <th style={{ textAlign: "left", padding: 6, borderBottom: `1px solid ${C.border}` }}>Prueba</th>
                    <th style={{ padding: 6, borderBottom: `1px solid ${C.border}` }}>Pre (prom)</th>
                    <th style={{ padding: 6, borderBottom: `1px solid ${C.border}` }}>Post (prom)</th>
                  </tr></thead>
                  <tbody>
                    {[{ l: "Rosenberg", t: "rosenberg", f: "total" }, { l: "MSPSS", t: "mspss", f: "total" }, { l: "AETDC", t: "aetdc", f: "total" }].map((r) => (
                      <tr key={r.t}>
                        <td style={{ padding: 6, borderBottom: `1px solid ${C.border}`, fontWeight: 600 }}>{r.l}</td>
                        <td style={{ padding: 6, borderBottom: `1px solid ${C.border}`, textAlign: "center", color: C.accent }}>{avg(instStudents, "pre", r.t, r.f)}</td>
                        <td style={{ padding: 6, borderBottom: `1px solid ${C.border}`, textAlign: "center", color: C.green }}>{avg(instStudents, "post", r.t, r.f)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div style={{ maxHeight: 300, overflowY: "auto" }}>
                  {instStudents.map((s) => {
                    ...
                    return (
                      <div key={s.doc} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${C.border}`, fontSize: 12 }}>
                        <div><span style={{ fontWeight: 600 }}>{s.name}</span>{s.salon && <span style={{ color: C.accent, marginLeft: 8, fontSize: 11 }}>📎 {s.salon}</span>}</div>
